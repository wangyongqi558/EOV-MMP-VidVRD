...
__all__ = ["load_model", "load_video", "load_frames", "tokenize"]
...
def load_frames(frames):
    ...
    video = [torch.from_numpy(np.array(Image.open(frame))).unsqueeze(0) for frame in frames]
    ...
    trans = transforms.Compose([
        video_transform.TensorToNumpy(),
        video_transform.Resize((scale_size, scale_size)),
        video_transform.ClipToTensor(channel_nb=3),
        video_transform.Normalize(mean=input_mean, std=input_std)
    ])
